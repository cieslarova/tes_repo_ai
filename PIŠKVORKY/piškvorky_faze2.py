...
class Game:
    ...
    def run(self):
        ...
        Hra pokračuje, dokud není ukončena (zatím bez kontroly vítěze/remízy).
        """
        ...
        while not self.game_over:
            ...
            # Pokusí se provést tah. Pokud je tah platný, přepne hráče.
            if self.board.make_move(row, col, self.current_player.symbol):
                self.board.display() # Zobrazí aktualizovanou plochu.
                # V této fázi ještě není implementována kontrola vítěze nebo remízy.
                # Pro demonstraci se hra po několika tazích ukončí, nebo bude pokračovat donekonečna.
                # Prozatím se přepíná hráč po každém platném tahu.
                self.switch_player()
            # Pokud tah nebyl platný, hráč je vyzván k opakování tahu a hráč se nepřepíná.

            # Pro účely testování fáze 2, aby se smyčka neopakovala donekonečna,
            # lze zde přidat podmínku pro ukončení, např. po 5 tazích.
            # V plné hře bude ukončena, až bude vítěz nebo remíza.
            # self.game_over = True # Odkomentujte pro okamžité ukončení po prvním tahu.
